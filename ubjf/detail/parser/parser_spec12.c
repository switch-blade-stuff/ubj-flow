...
static void ubjf_s12_guarded_read(ubjf_s12_parse_ctx *restrict ctx, void *restrict dest, size_t n)
{
	if (UBJF_UNLIKELY(!(ctx->read_info.read && ctx->read_info.read(dest, n, ctx->read_info.udata) == n)))
		THROW_ERROR(ctx->panic_buf, UBJF_EOF);
}
static char ubjf_s12_guarded_peek(ubjf_s12_parse_ctx *ctx)
{
	int result;
...
static void ubjf_s12_guarded_bump(ubjf_s12_parse_ctx *ctx, size_t n)
{
	if (UBJF_UNLIKELY(!(ctx->read_info.bump && ctx->read_info.bump(n, ctx->read_info.udata))))
		THROW_ERROR(ctx->panic_buf, UBJF_EOF);
}

static ubjf_token ubjf_s12_read_token(ubjf_s12_parse_ctx *ctx)
{
	ubjf_token token = 0;
	ubjf_s12_guarded_read(ctx, &token, 1);
	return token;
}
static ubjf_token ubjf_s12_peek_token(ubjf_s12_parse_ctx *ctx) { return (ubjf_token) ubjf_s12_guarded_peek(ctx); }

/* Parse event invocation. */
static void ubjf_s12_invoke_on_value(ubjf_s12_parse_ctx *ctx, ubjf_value value)
{
...
static char *ubjf_s12_invoke_string_alloc(ubjf_s12_parse_ctx *ctx, size_t size)
{
	char *str;
...
static void ubjf_s12_invoke_on_container_begin(ubjf_s12_parse_ctx *ctx, ubjf_type container_type, size_t length,
...
static void ubjf_s12_invoke_on_container_end(ubjf_s12_parse_ctx *ctx)
{
...
static void ubjf_s12_parse_node(ubjf_s12_parse_ctx *ctx, ubjf_type type);
static void ubjf_s12_read_node_recursive(ubjf_s12_parse_ctx *ctx);

static void ubjf_s12_parse_integer(ubjf_s12_parse_ctx *restrict ctx, ubjf_value *restrict value)
{
	switch (value->type)
	{
		...
		{
			uint8_t temp;
			ubjf_s12_guarded_read(ctx, &temp, sizeof(temp));
			value->uint8 = temp;
			break;
		}
		case UBJF_INT16:
...
static void ubjf_s12_parse_float(ubjf_s12_parse_ctx *restrict ctx, ubjf_value *restrict value)
{
	switch (value->type)
	{
		case UBJF_FLOAT32:
...
static int64_t ubjf_s12_parse_length(ubjf_s12_parse_ctx *ctx)
{
...
static void ubjf_s12_parse_string(ubjf_s12_parse_ctx *restrict ctx, ubjf_value *restrict value)
{
	...
	{
		ubjf_s12_guarded_read(ctx, str, length);
...
static void ubjf_s12_skip_highp(ubjf_s12_parse_ctx *ctx)
{
	int64_t length = ubjf_s12_parse_length(ctx);
	ubjf_s12_guarded_bump(ctx, length);
}
static void ubjf_s12_parse_value(ubjf_s12_parse_ctx *ctx, ubjf_type type)
{
	...
	if ((type & UBJF_BOOL_TYPE_MASK) == UBJF_BOOL_TYPE_MASK)
	{
		...
		ubjf_s12_parse_string(ctx, &value);
	...
	ubjf_s12_invoke_on_value(ctx, value);
}

static void ubjf_s12_parse_sized_container_next(ubjf_s12_parse_ctx *ctx, ubjf_type value_type)
{
...
static void ubjf_s12_parse_array(ubjf_s12_parse_ctx *ctx, int64_t length, ubjf_type value_type)
{
	if (length >= 0)
		while (length-- > 0)
		{
...
static void ubjf_s12_parse_object(ubjf_s12_parse_ctx *ctx, int64_t length, ubjf_type value_type)
{
	if (length >= 0)
		while (length-- > 0)
		{
		...
		for (;;)
		{
			if (ubjf_s12_guarded_peek(ctx) == UBJF_TOKEN_OBJECT_END)
			{
				ubjf_s12_guarded_bump(ctx, 1);
				break;
			}
...
static void ubjf_s12_parse_container_preface(ubjf_s12_parse_ctx *restrict ctx,
...
static void ubjf_s12_parse_container(ubjf_s12_parse_ctx *ctx, ubjf_type type)
{
	...
	ubjf_s12_invoke_on_container_begin(ctx, type, length, value_type);
	...
	END_GUARD(ctx->panic_buf);
	ubjf_s12_invoke_on_container_end(ctx);
}
