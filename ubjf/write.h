...
typedef struct {
  ...
  /** Optional fixed length for container's value. To make container
   * dynamically-size, set to `-1`. */
  int64_t length;
  /** Optional fixed type for container's value. To make container
   * dynamically-typed, set to `UBJF_NO_TYPE`.
   * @note If a fixed type is set, fixed length must be provided as well.
   * @note For fixed-type containers, `UBJF_BOOL` will be ignored. Use
   * `UBJF_TRUE_TYPE` and `UBJF_FALSE_TYPE` instead. */
 ...
 * @return On success, returns `UBJF_NO_ERROR`. On error, returns the error
 * code. */
UBJF_EXTERN ubjf_error ubjf_init_write(ubjf_write_state *state,
                                       ubjf_write_state_info init_info);
 ...
 * @return On success, returns `UBJF_NO_ERROR`. On error, returns the error
 * code. */
UBJF_EXTERN ubjf_error
ubjf_init_file_write(ubjf_write_state *UBJF_RESTRICT state,
                     ubjf_write_state_info init_info, FILE *UBJF_RESTRICT file);
 ...
 * @return On success, returns `UBJF_NO_ERROR`. On error, returns the error
 * code. */
UBJF_EXTERN ubjf_error ubjf_init_buffer_write(
    ubjf_write_state *UBJF_RESTRICT state, ubjf_write_state_info init_info,
 ...
 * @return On success, returns `UBJF_NO_ERROR`. On error, returns the error
 * code. */
UBJF_EXTERN ubjf_error ubjf_write_value(ubjf_write_state *state,
                                        ubjf_value value);
...
static inline ubjf_error ubjf_write_bool(ubjf_write_state *state, bool b) {
  ubjf_value value = {.boolean = b, .type = UBJF_BOOL};
  return ubjf_write_value(state, value);
}
...
static inline ubjf_error ubjf_write_char(ubjf_write_state *state, char c) {
  ubjf_value value = {.character = c, .type = UBJF_CHAR};
  return ubjf_write_value(state, value);
}
...
static inline ubjf_error ubjf_write_int8(ubjf_write_state *state, int8_t i) {
  ubjf_value value = {.integer = i, .type = UBJF_INT8};
  return ubjf_write_value(state, value);
}
...
static inline ubjf_error ubjf_write_uint8(ubjf_write_state *state, uint8_t i) {
  ubjf_value value = {.integer = i, .type = UBJF_UINT8};
  return ubjf_write_value(state, value);
}
...
static inline ubjf_error ubjf_write_int16(ubjf_write_state *state, int16_t i) {
  ubjf_value value = {.integer = i, .type = UBJF_INT16};
  return ubjf_write_value(state, value);
}
...
static inline ubjf_error ubjf_write_int32(ubjf_write_state *state, int32_t i) {
  ubjf_value value = {.integer = i, .type = UBJF_INT32};
  return ubjf_write_value(state, value);
}
...
static inline ubjf_error ubjf_write_int64(ubjf_write_state *state, int64_t i) {
  ubjf_value value = {.integer = i, .type = UBJF_INT64};
  return ubjf_write_value(state, value);
}
...
static inline ubjf_error ubjf_write_float32(ubjf_write_state *state, float i) {
  ubjf_value value = {.floating = i, .type = UBJF_INT32};
  return ubjf_write_value(state, value);
}
...
static inline ubjf_error ubjf_write_float64(ubjf_write_state *state, double i) {
  ubjf_value value = {.floating = i, .type = UBJF_INT64};
  return ubjf_write_value(state, value);
}
 ...
 * @return On success, returns `UBJF_NO_ERROR`. On error, returns the error
 * code. */
static inline ubjf_error ubjf_write_highp(ubjf_write_state *state,
                                          ubjf_string h) {
  ubjf_value value = {.highp = h, .type = UBJF_HIGHP};
  return ubjf_write_value(state, value);
}
 ...
 * @return On success, returns `UBJF_NO_ERROR`. On error, returns the error
 * code. */
static inline ubjf_error ubjf_write_string(ubjf_write_state *state,
                                           ubjf_string str) {
  ubjf_value value = {.string = str, .type = UBJF_STRING};
  return ubjf_write_value(state, value);
}
 ...
 * @return On success, returns `UBJF_NO_ERROR`. On error, returns the error
 * code.
 * @note If a container info is not valid, call to this function will fail with
 * error being set to `UBJF_ERROR_PARAM` with index 1. */
UBJF_EXTERN ubjf_error ubjf_start_container(ubjf_write_state *state,
 ...
 * @return On success, returns `UBJF_NO_ERROR`. On error, returns the error
 * code. */
UBJF_EXTERN ubjf_error ubjf_write_object_key(ubjf_write_state *state,
                                             ubjf_string key);
/** Ends a previously started container write.
 * @param[in] state State to use for writing.
 * @return On success, returns `UBJF_NO_ERROR`. On error, returns the error
 * code.
 * @note If a container write was not started, call to this function will fail
 * with error being set to `UBJF_ERROR_PARAM` with index 0. */
 ...
 * @return On success, returns `UBJF_NO_ERROR`. On error, returns the error
 * code.
 * @note If a fixed type is specified, every value within the source array must
 * be of the same type. */
UBJF_EXTERN ubjf_error ubjf_write_array(ubjf_write_state *UBJF_RESTRICT state,
                                        const ubjf_value *UBJF_RESTRICT data,
